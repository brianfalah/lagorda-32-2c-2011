...
int traducirSectorAUnsignedInt(unsigned char *sector){
	int sectorInt=0;
	//char sectConMenosBin[9];
	char *sectConMenosBin = malloc(8 + 1);
    char *sectorBin = malloc(8 + 1);
    char *sectorFinal = malloc(32 + 1);
	char *end;
	unsigned int sectFinal=0;

	sectorInt=sector[0];
	itoa(sectorInt,sectConMenosBin,2);
	sprintf(sectorBin,"%08d",atoi(sectConMenosBin));
	strcpy(sectorFinal,sectorBin);
	sectorInt=sector[1];
	itoa(sectorInt,sectConMenosBin,2);
	sprintf(sectorBin,"%08d",atoi(sectConMenosBin));
	strcat(sectorFinal,sectorBin);
	sectorInt=sector[2];
	itoa(sectorInt,sectConMenosBin,2);
	sprintf(sectorBin,"%08d",atoi(sectConMenosBin));
	strcat(sectorFinal,sectorBin);
	...
	sprintf(sectorBin,"%08d",atoi(sectConMenosBin));
	strcat(sectorFinal,sectorBin);
	sectFinal=(unsigned int)strtoul(sectorFinal,&end,2);

    free(sectConMenosBin);
    free(sectorBin);
    free(sectorFinal);

	return (unsigned int) sectFinal;
}
